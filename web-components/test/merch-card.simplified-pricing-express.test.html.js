    ...
    describe('merch-card simplified-pricing-express variant', () => {
        ...
        it('should display heading in correct slot', async () => {
            ...
            expect(heading.textContent).to.equal(
                'Express - Create standout content',
            );
        });
        ...
        it('should display badge with custom colors', async () => {
            ...
            expect(card.getAttribute('badge-background-color')).to.equal(
                '#FFD700',
            );
        ...
        it('should display price when provided', async () => {
            const cardWithPrice = document.querySelectorAll(
                'merch-card[variant="simplified-pricing-express"]',
            )[1];
        ...
        it('should support custom border color', async () => {
            const cardWithBorder = document.querySelector(
                'merch-card[border-color="#E91E63"]',
            );
            expect(cardWithBorder).to.exist;
            expect(cardWithBorder.getAttribute('border-color')).to.equal(
                '#E91E63',
            );
            ...
            const borderColorVar = computedStyle.getPropertyValue(
        ...
        it('should support multiple CTAs', async () => {
            const cardWithMultipleCtas = document.querySelectorAll(
                'merch-card[variant="simplified-pricing-express"]',
            )[2];
        ...
        it('should handle badge when no background color provided', async () => {
            const cardNoBadgeBg = document.querySelector(
                'merch-card[border-color="#FF5722"]',
            );
            expect(cardNoBadgeBg.hasAttribute('badge-background-color')).to.be
                .false;
    ...
    describe('Tooltip functionality in simplified-pricing-express', () => {
        describe('Basic tooltip rendering', () => {
            it('should render mas-mnemonic in price slot', async () => {
                ...
                expect(masMnemonic.getAttribute('content')).to.equal(
                    'Price after first year at regular rate',
                );
            ...
            it('should render mas-mnemonic with icon in description', async () => {
                ...
                expect(masMnemonic).to.exist;
                expect(masMnemonic.getAttribute('content')).to.equal(
                    'Including Photoshop, Illustrator, Premiere Pro, After Effects, and more',
                );
            ...
            it('should handle multiple tooltips in the same card', async () => {
                ...
                expect(tooltips[0].getAttribute('content')).to.equal(
                    'Sync across all your devices',
                );
                expect(tooltips[1].getAttribute('content')).to.equal(
                    'Access to 1000+ exclusive designs',
                );
                expect(tooltips[2].getAttribute('content')).to.equal(
                    'Remove background, resize, and more',
                );
            ...
            it('should render icon-based tooltip without slot content', async () => {
                ...
                expect(masMnemonic).to.exist;
                expect(masMnemonic.getAttribute('src')).to.include(
                    'info-icon.svg',
                );
                expect(masMnemonic.getAttribute('size')).to.equal('s');
                expect(masMnemonic.getAttribute('tooltip-text')).to.equal(
                    'Perfect for teams and enterprises',
                );
                expect(masMnemonic.getAttribute('tooltip-placement')).to.equal(
                    'bottom',
                );
            ...
            it('should support different tooltip placements', async () => {
                ...
                tooltips.forEach((tooltip, index) => {
                    expect(tooltip.getAttribute('placement')).to.equal(
                        placements[index],
                    );
                    expect(tooltip.getAttribute('content')).to.equal(
                        `${placements[index].charAt(0).toUpperCase() + placements[index].slice(1)} placement`,
                    );
                });
            });
        });
        ...
        it('should preserve tooltips during card updates', async () => {
            ...
            expect(tooltipAfterUpdate.getAttribute('content')).to.equal(
                'Price after first year at regular rate',
            );
        });
    });
