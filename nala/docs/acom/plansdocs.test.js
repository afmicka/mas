...
test.describe('ACOM MAS cards feature test suite', () => {
    ...
    test(`${features[1].name},${features[1].tags}`, async () => {
        ...
        await test.step('step-2: Verify Plans Merch Card CSS', async () => {
            ...
            expect(
                await webUtil.verifyCSS(await acomPage.getCardDescription(data.id).first(), acomPage.cssProp.description),
            ).toBeTruthy();
            expect(
                await webUtil.verifyCSS(await acomPage.getSeeAllPlansLink(data.id), acomPage.cssProp.legalLink),
            ).toBeTruthy();
            ...
            expect(
                await webUtil.verifyCSS(await acomPage.getCardSecureTransaction(data.id), acomPage.cssProp.secureTransaction),
            ).toBeTruthy();
