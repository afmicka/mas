...
import { features, MINICOMPARE_MWEB_FRAGMENT_IDS } from './minicomparemwebdocs.spec.js';
import MasMinicompareMweb from './minicomparemweb.page.js';
...
const workerSetup = createWorkerPageSetup({
    pages: [{ name: 'US', url: DOCS_GALLERY_PATH.MINICOMPARE_MWEB }],
});

test.describe('Mini Compare Chart Mweb gallery feature test suite', () => {
    ...
    test(`[Test Id - ${features[0].tcid}] ${features[0].name},${features[0].tags}`, async () => {
        ...
        await test.step('step-1: Go to Mini Compare Chart Mweb gallery page', async () => {
            const page = workerSetup.getPage('US');
            galleryPage = new MasMinicompareMweb(page);
            await workerSetup.verifyPageURL('US', DOCS_GALLERY_PATH.MINICOMPARE_MWEB, expect);
        });

        await test.step('step-2: Verify gallery heading', async () => {
            await expect(galleryPage.getGalleryHeading()).toBeVisible();
            await expect(galleryPage.getGalleryHeading()).toHaveText('Mini Compare Chart Gallery');
        });

        await test.step('step-3: Verify Mweb cards and AEM fragment ids', async () => {
            const cards = galleryPage.getMwebCards();
            await expect(cards).toHaveCount(MINICOMPARE_MWEB_FRAGMENT_IDS.length);
            for (const id of MINICOMPARE_MWEB_FRAGMENT_IDS) {
        ...
        await test.step('step-4: Verify sample card hydrated content', async () => {
            ...
            await expect(card.locator('div[slot="body-m"]')).toBeVisible();
            await expect(card.locator('div[slot="body-m"]')).not.toHaveText('');
        });
    });
